...
| `scheduler.py` | APScheduler 3.x `AsyncIOScheduler`, four jobs: every-minute tick (`_scheduled_check` — runs the check cycle, then initializes one NEW topic), stuck-topic recovery (every 5 min, 15-min timeout), weekly VACUUM (Sun 3 AM), daily article + delivery/check-intent ledger cleanup (4 AM). Only the minute tick has jitter; all jobs coalesce and are single-instance. One process at a time runs a scheduler: `start_scheduler()` takes an advisory lock on `scheduler.lock` beside the database and returns `None` without one, so extra web workers serve requests only. Per-job outcomes (last success/error, consecutive failures, missed runs) are recorded and reported by `/health`. The check cycle (`_run_check_cycle`) snapshots due topics, then drains pending notification/webhook retries and resumes accepted check intents *alongside* the per-topic checks in one `asyncio.gather()` rather than ahead of them, so a retry backlog cannot delay a tick's due-topic work. The lease is `fcntl.flock`, so it is enforced on POSIX only; on a platform without `fcntl` the acquire is a no-op and the documented one-worker rule is all there is. Each due topic runs under `CHECK_TIMEOUT_SECONDS` (540 s, under the 600 s guard-eviction threshold); the whole-cycle deadline (`_CHECK_ALL_TIMEOUT_SECONDS` plus the backlog's share) wraps only the web UI's Check All Now — the scheduler tick relies on the per-topic bound plus APScheduler `max_instances=1` / `coalesce=True`; the CLI on the per-topic bound alone. |
...
| `clock.py` | The process's monotonic clock as one seam: `monotonic_now()`. In-process liveness — how long a check guard has been held, how much of a provider cooldown is left, how old a rate-limit window entry is, how fresh a scheduler job's last run is — is measured on it, never by differencing wall-clock readings, so a host clock step cannot expire or extend it. Consumers: `web/state.py`, `scraping/routing.py`, `scheduler.py`; other monotonic timers (`scraping/google_news.py`'s rate-limit cooldown, `analysis/llm.py`'s retry budget and mode-hint TTL, `scraping/source.py`'s `Deadline`) still read `time.monotonic()` directly. Durable due-times (`feed_backoff.py`, delivery-claim leases) deliberately stay wall clock: a monotonic reading means nothing across processes. Tests patch `app.clock.monotonic_now` once. |
...
**Monotonic for liveness, wall clock for durability.** Anything measured inside one process — how long a check guard has been held, how much of a provider cooldown is left, how old a rate-limit window entry is, how fresh a scheduler job's last run is — reads `clock.monotonic_now()`, so a host clock step cannot expire or extend it, while anything that has to outlive the process is a wall-clock instant. The delivery claim is the one place both appear: `claimed_at` is wall clock because a claim left behind by a dead process must still be recoverable and no monotonic reading survives it, so a process-local set of live claim tokens — not the clock — is what keeps a running sender's claim from being re-armed and sent twice underneath it (AUG-277).
...
2. `_run_check_cycle()` snapshots the due topics on one short connection, then runs the notification/webhook/check-intent retry drain and every due topic's check *concurrently* in one `asyncio.gather()` — the drain no longer runs ahead of due-topic work, so a retry backlog cannot delay a whole tick. `retry_pending_notifications()` / `retry_pending_webhooks()` / `retry_pending_check_intents()` each manage their own short-lived connections and commit per item.
3. `get_topics_due_for_check()` finds active READY topics whose last check exceeds their interval. It anchors each topic on its newest check that is not stamped in the future (more than `FUTURE_ANCHOR_SKEW_MINUTES` ahead of now); a future-stamped row is skipped and logged, so one impossible timestamp cannot park a topic until that time passes (AUG-278).
...
4. The background task claims it and runs `check_topic()`; a row a dead process left behind is re-armed and run by the next scheduler check cycle (after the 10-minute claim lease when the process died mid-check) — the page's poll may have ended by then, so the result shows on the next load. Check All Now is not admitted this way: it runs one scheduler cycle, and the next minute tick is its resume.
